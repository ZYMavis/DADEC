...
static void merge_hits(step_t *s)
{
    ...
    qlens = (int *)calloc(max_seg + s->p->n_parts * 3, sizeof(int));
	...
	for (f = 0, k = k0 = 0; f < s->n_frag; ++f) {
		...
		for (i = 0; i < s->n_seg[f]; ++i, ++k) {
            ...
            s->reg[k] = (mm_reg1_t *)calloc(s->n_reg[k], sizeof(mm_reg1_t));
...
int mm_split_merge(int n_segs, const char **fn, const mm_mapopt_t *opt, int n_split_idx)
{
    ...
    pl.fp_parts = (FILE **)calloc(pl.n_parts, sizeof(FILE *));
    pl.rid_shift = (uint32_t *)calloc(pl.n_parts, sizeof(uint32_t));
...
static bool encodeCigarAndBase(int &baseCode,uint32_t cigar,int &pos,char* read){
	uint32_t snp_len=cigar>>4;
	char type=MM_CIGAR_STR[cigar&0xf];
	if(type=='='){
		pos+=snp_len;
		return false;
	}
	else if(type=='I'||type=='X'){
		for(int k=0;k<snp_len;k++){
			if(k<8){
				int b;
				//cout<<read[pos]<<endl;
				if(read[pos]=='A') b=0;
				else if(read[pos]=='T') b=1;
				else if(read[pos]=='G') b=2;
				else if(read[pos]=='C') b=3;
			
				baseCode=(baseCode<<2)|b;
			}
			pos+=1;
		}
		return true;
...
static void *process_sequences(void *shared, int step, void *in)
{
	int i, j, k;
    pipeline_t *p = (pipeline_t*)shared;
    if (step == 0) { // step 0: read sequences
      ...
      step_t *s = (step_t*)in;
		  ...
		  for (i = 0; i < p->n_threads; ++i) mm_tbuf_destroy(s->buf[i]);
		  free(s->buf);
      FILE *fp = fopen(alnFilename.c_str(), "a");
      if (fp == NULL) {
		...
		for (k = 0; k < s->n_frag; ++k) {
			...
			for (i = seg_st; i < seg_en; ++i) {
				mm_bseq1_t *t = &s->seq[i];
				if (s->n_reg[i] > 0) { // the query has at least one hit
					for (j = 0; j < s->n_reg[i]; ++j) {         
						mm_reg1_t *r = &s->reg[i][j];  
						assert(!r->sam_pri || r->id == r->parent);
						if ((p->opt->flag & MM_F_NO_PRINT_2ND) && r->id != r->parent)
						...
						char qt = "+-"[r->rev];
						int pos=0;
						char* longName=mi->seq[r->rid].name;
						int longId = atoi(longName + 2);
						int shortId=atoi(t->name + 2)-1;
						fprintf(fp, "%u %u %c %u %u %u %u %u %u %u",
							longId , shortId , qt , r->rs, r->re,
							r->qs, r->qe, r->p->dp_score, r->mlen, r->p->n_cigar);
						std::unique_ptr<char[]> sread(new char[t->l_seq + 1]);
						if (qt == '-') {
							correctRead::reverse(sread.get(), t->seq, t->l_seq);
						} else {
							std::strncpy(sread.get(), t->seq,t->l_seq);
						}
						sread[t->l_seq] = '\0';
						for (uint32_t m = 0; m < r->p->n_cigar; ++m) {
							fprintf(fp, " %u", r->p->cigar[m]);
							int baseCode=0;
							if(encodeCigarAndBase(baseCode,r->p->cigar[m],pos,sread.get()))
								fprintf(fp, " %u", baseCode);
						}
						fprintf(fp, "\n");
					}
				}
...
int mm_map_file_frag1(const mm_idx_t *idx, int n_segs, const char **fn, const mm_mapopt_t *opt, int n_threads)
{	
  ...
  //pl.snum=short_reads_number;
	if (opt->split_prefix)
		pl.fp_split = mm_split_init(opt->split_prefix, idx);
	pl_threads = n_threads == 1? 1 : (opt->flag&MM_F_2_IO_THREADS)? 3 : 2;
	
	kt_pipeline(pl_threads, process_sequences, &pl, 3);
...
void rManage::runMinimap(const char *target_file,const char *query_file, string& AlnFilename) {
	alnFilename=AlnFilename;
    string sortedFile=alnFilename;
	sortedFile.insert(sortedFile.size()-4,"sort");
	if(!std::filesystem::exists(sortedFile)){
		if(!std::filesystem::exists(alnFilename)){
			mm_idxopt_t iopt;
			mm_mapopt_t mopt;
			int n_threads = 64;
			...
			mopt.flag |= MM_F_SR  ;
			iopt.batch_size=iopt.batch_size*2;
			...
			while ((mi = mm_idx_reader_read(r, n_threads)) != 0) {
				cout<<"long_reads_num:"<<mi->n_seq<<endl;
				mm_mapopt_update(&mopt, mi);
				mm_tbuf_t *tbuf = mm_tbuf_init();
				int short_reads_id=0;
				cout<<"Start comparison with minimap..."<<endl;
				mm_map_file_frag1(mi, 1, &query_file, &mopt, n_threads);
		...
		cout<<"The comparison file has been sorted!"<<endl;
	}
    AlnFilename=sortedFile;
}
