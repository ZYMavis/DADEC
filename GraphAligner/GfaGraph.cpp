...
GfaGraph GfaGraph::LoadFromFaFile(std::string filename,int kmerSize)
{
	if (filename.substr(filename.size()-3) == ".gz")
	{
		zstr::ifstream file { filename };
		return LoadFromFaStream(file,kmerSize);
	}
	else
	{
		std::ifstream file { filename };
...
GfaGraph GfaGraph::LoadFromFaStream(std::istream& file,int kmerSize)
{
	...
	while (file.good()){
		...
		while(sstr>>path){
			...
			std::string toend;
			std::string overlapstr=std::to_string(kmerSize-1)+"M";
			int overlap = 0;
